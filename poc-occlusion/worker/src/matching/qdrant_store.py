...
class QdrantPersonStore:
    ...
    def get_embedding(self, person_id: int) -> np.ndarray | None:
        """Retrieve the canonical embedding for a person."""
    ...
    def gated_momentum_update(
        ...
        """Gated canonical embedding update.

        Only updates if ALL conditions are met:
        1. v_avg is high enough
        2. Embedding consistency is good
        3. Tracklet is long enough
        4. New embedding is not too divergent from canonical
        ...
        # Get current canonical embedding
        canonical = self.get_embedding(person_id)
        if canonical is None:
            return False

        # Check similarity to canonical (not too divergent)
        sim = float(np.dot(canonical, new_embedding))
        if sim < update_sim_threshold:
            return False

        # All gates passed — apply momentum update
