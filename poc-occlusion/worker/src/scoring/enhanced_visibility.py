"""Enhanced visibility scoring at worker level.

Augments edge-computed v_score with temporal cues from the tracker.
"""
...
def compute_iou_prev(bbox_curr: np.ndarray, bbox_prev: np.ndarray | None) -> float:
    """Score based on IoU with previous frame's bbox. High IoU = stable = good.
...
def compute_vel_smooth(
    ...
    """Detect sudden jumps in bbox center relative to bbox size.
    ...
    disp_score = 1.0 if normalized_disp < 0.3 else (0.6 if normalized_disp < 0.5 else 0.3)
    accel_score = 1.0 if accel < 0.2 else (0.6 if accel < 0.4 else 0.3)
...
def compute_v_worker(
    ...
    """Compute enhanced worker-level visibility score.

    v_worker = 0.60 * v_edge + 0.25 * iou_prev + 0.15 * vel_smooth
    """
    w = weights or {"v_edge": 0.60, "iou_prev": 0.25, "vel_smooth": 0.15}
    return (
        w["v_edge"] * v_edge
        + w["iou_prev"] * iou_prev_score
        + w["vel_smooth"] * vel_smooth_score
    )
