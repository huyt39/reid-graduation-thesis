...
from src.models.gem_pooling import GeM
...
class OSNet(nn.Module):
    """Omni-Scale Network with GeM pooling for person Re-ID."""

    def __init__(self, num_classes, blocks, layers, channels, feature_dim=512, loss="softmax", IN=False, gem_p=3.0, **kwargs):
        super().__init__()
        ...
        # GeM pooling instead of AdaptiveAvgPool2d(1)
        self.global_avgpool = GeM(p=gem_p)
...
def init_pretrained_weights(model, key, weights_dir="weights"):
    """Load pretrained weights, matching layers by name and size."""
    weight_path = os.path.join(weights_dir, "osnet_x1_0.pth.tar")
    if not os.path.exists(weight_path):
        print(f"[Warning] Pretrained weights not found at {weight_path}, using random init")
        return
    ...
    print(f"Loaded {len(matched_layers)} pretrained layers, discarded {len(discarded_layers)}")


def osnet_x1_0(num_classes=1000, pretrained=True, loss="softmax", gem_p=3.0, weights_dir="weights", **kwargs):
    model = OSNet(
        num_classes,
    ...
    if pretrained:
        init_pretrained_weights(model, key="osnet_x1_0", weights_dir=weights_dir)
    return model
