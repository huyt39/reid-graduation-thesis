...
# ── Weight loading (vendored: load our own ReID checkpoint, no gdown) ──

def load_pretrained_weights(model, weight_path, device):
    """Load a ReID checkpoint into the model, matching by name+shape and
    discarding mismatched keys (e.g. the classifier sized for a different
    number of training identities). Mirrors the loader in osnet.py."""
    import sys
    from collections import OrderedDict
    import numpy as np
    # Compat alias for checkpoints pickled against newer numpy module paths.
...
def osnet_ain_x1_0(num_classes=1000, loss="softmax", weight_path=None, device=None):
    """OSNet-AIN x1.0 — domain-generalization variant (AdaIN). Kept faithful to
    torchreid so the validated MSMT17 generalization checkpoint loads correctly."""
    if device is None:
