...
import type { FrameUpdate, TrackedPerson } from "@/hooks/useWebSocket";
...
function drawOverlay(
  ...
  for (const p of persons) {
    ...
    // bbox may be pixel coords or 0-1 fractions — detect by magnitude
...
export default function LiveFeed({ frame }: Props) {
  ...
  if (!frame) {
    return (
      <div className="flex-1 flex items-center justify-center bg-panel rounded-xl border border-border">
        <p className="text-gray-500 text-sm">Waiting for frames…</p>
      </div>
    );
  }

  return (
    <div className="flex-1 relative overflow-hidden rounded-xl border border-border bg-black">
