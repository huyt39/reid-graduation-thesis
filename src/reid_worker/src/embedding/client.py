...
class ModelServiceClient:
    ...
    async def extract_features(
        ...
        data = {"model": model}
        response = await self.client.post(f"{self.base_url}/embedding", files=files, data=data)
        if response.status_code == 200:
            return original_idx, response.json()
        raise Exception(f"Feature extraction failed: {response.status_code}: {response.text}")
    ...
    async def extract_features_batch(
        ...
        data = {"model": model}
        response = await self.client.post(f"{self.base_url}/embedding/batch", files=files, data=data)
        if response.status_code == 200:
